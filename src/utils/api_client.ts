...
async function refreshToken(
  ...
  try {
    ...
    return {
      ...
      refreshToken: data.data?.refresh_token || tokenToRefresh,
      expiresAt,
    }
  } catch (error) {
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        ...
        const refreshTokenValue = session?.refreshToken as string
        ...
        if (refreshTokenValue) {
          ...
          if (refreshed) {
            ...
            if (retryResponse.status === 401) {
              console.log(
                '[API] 🚪 Retry still failed with 401 - redirecting to login:',
                {
                  endpoint,
          ...
          console.log(
            '[API] 🚪 No refresh token available - redirecting to login:',
            {
              endpoint,
